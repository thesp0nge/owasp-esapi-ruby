module Owasp
  module Esapi
    ...
    describe "-FileContent Tests-" do
      it "should allow this mock data with input size less than 1024 bytes" do
        io = StringIO.new("this is some junk")
        validator.valid_file_contents?("test",io,100,false).should be_true
      end
      it "should fail this mock data with input size greater than 10 bytes" do
        io = StringIO.new("this is some junk")
        validator.valid_file_contents?("test",io,10,false).should be_false
      end

    end

    describe "-Number Tests-" do
      # floats
      {
        "1.0" => [0,20,true],
        "3.222" => [0,20,true],
        ...
        "-10.334" => [-20,20,true],
        "1" => [0,10,true]
      }.each_pair do |val,rule|
        min = rule[0]
        max = rule[1]
        correct = rule[2]
        it "should check float value #{val} as #{correct} within #{min} and #{max}" do
          validator.valid_float?("test",val,min,max,false).should == correct
        end
      end
      #integers
      {
    ...
    describe "-FileName Tests-" do
      ...
      it "should check filename.junk as invalid with default allowed extension" do
        validator.valid_file_name?("test","filename.junk",nil,false).should be_false
      end
      ...
      }.each_pair do |file,value|
        it "should validate #{file} with default extensions should return #{value}" do
          validator.valid_file_name?("test",file,Esapi.security_config.allowed_extenions,false).should == value
        end
      end
